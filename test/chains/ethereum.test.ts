import {describe, it} from 'mocha'
import {expect} from 'chai'
import {validate} from '../../src'

describe('ethereum', () => {
    it('should accept valid addresses', () => {
        const valid = [
            '0xde709f2102306220921060314715629080e2fb77',
            '0x27b1fdb04752bbc536007a920d24acb045561c26',
            '0xc6d9d2cd449a754c494264e1809c50e34d64562b',
            '0x52908400098527886E0F7030069857D2E4169EE7',
            '0x8617E340B3D01FA5F11F306F4090FD50E238070D',
            '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776D9FF',
            '0xa00354276d2fC74ee91e37D085d35748613f4748',
            '0xAff4d6793F584a473348EbA058deb8caad77a288',
            '0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAed',
            '0xfB6916095ca1df60bB79Ce92cE3Ea74c37c5d359',
            '0xdbF03B407c01E7cD3CBea99509d93f8DDDC8C6FB',
            '0xD1220A0cf47c7B9Be7A2E6BA89F429762e7b9aDb',
        ]
        valid.forEach(addr => {
            expect(validate(addr, 'ethereum'), `expected ${addr} to be valid`).to.equal(true)
        })
    })

    it('should reject invalid addresses', () => {
        const invalid = [
            '',
            'notanaddress',
            'E37c0D48d68da5c5b14E5c1a9f1CFE802776D9FF',
            '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776D9F',
            '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776D9FFF',
            '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776ZZZZ',
            '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776D9Ff',
            '0x5aaeb6053F3E94C9b9A09f33669435E7Ef1BeAed',
            '12KYrjTdVGjFMtaxERSk3gphreJ5US8aUP',
            '0XE37C0D48D68DA5C5B14E5C1A9F1CFE802776D9FF',
        ]
        invalid.forEach(addr => {
            expect(validate(addr, 'ethereum'), `expected "${addr}" to be invalid`).to.equal(false)
        })
    })

    it('should accept alternative chain names', () => {
        const addr = '0xE37c0D48d68da5c5b14E5c1a9f1CFE802776D9FF';
        ['eth', 'arbitrum', 'avalanche', 'base', 'binance', 'bnb', 'bsc', 'optimism', 'monad'].forEach(name => {
            expect(validate(addr, name), `expected valid via name "${name}"`).to.equal(true)
        })
    })
})
